...
export default function OwnerLoginPage() {
  ...
  if (checkingAuth) {
    return (
      <main className="min-h-screen flex items-center justify-center bg-ink text-cream/50">
        Loading…
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-ink text-cream px-6 py-12">
        ...
        <div className="flex gap-1.5 mb-5">
          {[0, 1].map((i) => {
            ...
            return (
              <div
                key={i}
                className={`h-1 flex-1 rounded ${i <= stepIdx ? 'bg-gold' : 'bg-ink-light'}`}
              />
            );
          })}
        </div>
