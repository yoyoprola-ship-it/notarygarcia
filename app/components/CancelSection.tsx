...
export default function CancelSection({ lang }: { lang: Lang }) {
  ...
  if (step === 'idle') {
    return (
      <section id="cancel" className="px-6 py-10 bg-ink border-t border-gold/15">
        <div className="max-w-2xl mx-auto text-center">
          <button
            onClick={() => setStep('phone')}
            className="px-6 py-3 bg-ink-light border border-gold/25 hover:border-gold/50 hover:shadow-sm rounded-full text-sm font-bold uppercase tracking-wide text-cream/80 transition-all"
          >
            {t.trigger}
          </button>
        </div>
  ...
  return (
    <section id="cancel" className="px-6 py-16 bg-ink border-t border-gold/15">
          ...
          <>
            <div className="flex gap-1.5 mb-4">
              {[0, 1].map((i) => {
                ...
                return (
                  <div key={i} className={`h-1 flex-1 rounded ${i <= idx ? 'bg-gold' : 'bg-ink-light'}`} />
                );
              })}
            </div>
                  ...
                  return (
                    <div
                      ...
                      className={`border rounded-lg p-4 flex items-center justify-between gap-4 transition-opacity ${
                        isCancelled ? 'border-gold/10 opacity-50' : 'border-gold/25 bg-ink-light'
                      } ${isBusy ? 'opacity-60' : ''}`}
                    >
                      <div>
...
const inputCls =
  'w-full px-4 py-3 bg-ink-light border border-gold/25 rounded-md text-cream placeholder:text-cream/30 focus:outline-none focus:border-gold focus:ring-1 focus:ring-gold transition-colors';

const btnCls =
  'w-full px-4 py-3 bg-gold hover:bg-gold-light active:bg-gold text-ink-deep font-bold uppercase tracking-wide rounded-md transition-colors disabled:opacity-50';
